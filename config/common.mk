...
PRODUCT_PROPERTY_OVERRIDES += \
    ro.rommanager.developerid=candysix
endif
...
PRODUCT_COPY_FILES += \
    vendor/candy/CHANGELOG.mkdn:system/etc/CHANGELOG-CM.txt
...
PRODUCT_COPY_FILES += \
    ...
    vendor/candy/prebuilt/common/bin/50-cm.sh:system/addon.d/50-cm.sh \
...
PRODUCT_COPY_FILES += \
    vendor/candy/prebuilt/common/etc/init.local.rc:root/init.cm.rc
...
PRODUCT_VERSION_MINOR = alpha-0.1
...
ifeq ($(CANDY_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        CANDY_VERSION := $(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CANDY_BUILD)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            CANDY_VERSION := $(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(CANDY_BUILD)
        else
            CANDY_VERSION := $(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CANDY_BUILD)
        endif
    endif
...
PRODUCT_PROPERTY_OVERRIDES += \
  ro.candy.build.version.plat.sdk=$(CM_PLATFORM_SDK_VERSION)

# CyanogenMod Platform Internal
PRODUCT_PROPERTY_OVERRIDES += \
  ro.candy.build.version.plat.rev=$(CM_PLATFORM_REV)
